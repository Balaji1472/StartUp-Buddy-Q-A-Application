...
try:
    ...
    try:
        ...
        try:
            ...
            # Last resort - dummy functions
            def store_chat(question, response):
                return False
            def search_similar(query, top_k=3):
                return []
            st.sidebar.warning("Pinecone utils not imported - using dummy functions")
            pinecone_available = False
...
def initialize_langchain_memory():
    # Create a chat history and memory
...
if 'langchain_memory' not in st.session_state:
    try:
        st.session_state['langchain_memory'] = initialize_langchain_memory()
        st.sidebar.success("✅ LangChain memory initialized")
    except Exception as e:
        st.sidebar.error(f"❌ Error initializing LangChain memory: {str(e)}")
        st.session_state['langchain_memory'] = None
...
if submit and input_text and chat is not None:
    ...
    # Add the user message to LangChain memory
    if st.session_state['langchain_memory'] is not None:
        try:
            st.session_state['langchain_memory'].chat_memory.add_user_message(input_text)
            st.sidebar.success("Added user message to memory")
        except Exception as e:
            st.sidebar.error(f"Error adding to memory: {str(e)}")
    ...
    with st.spinner("Generating response..."):
        ...
        if response:
            ...
            if full_response:
                ...
                # Add the bot response to LangChain memory
                if st.session_state['langchain_memory'] is not None:
                    try:
                        st.session_state['langchain_memory'].chat_memory.add_ai_message(full_response)
                        st.sidebar.success("Added AI response to memory")
                        
                        # Debug: Show memory content
                        messages = st.session_state['langchain_memory'].chat_memory.messages
                        st.sidebar.write(f"Memory contains {len(messages)} messages")
